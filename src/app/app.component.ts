import { Component, IterableDiffers, SimpleChange } from '@angular/core';
...
export class AppComponent {
  isSubmitted = false;
  AVAILABLE_LIST_NAME = 'available';
  CONFIRMED_LIST_NAME = 'confirmed';
  LTR = 'left-to-right';
  RTL = 'right-to-left';
  DEFAULT_FORMAT = { add: 'Add', remove: 'Remove', all: 'All', none: 'None', direction: '', draggable: true };
  ...
  id = `dual-list-${nextId++}`;
	...
	height = '100px';
	filter = false;
	 format = { add: 'Add', remove: 'Remove', all: 'All', none: 'None', direction: '', draggable: true };
	sort = false;
	disabled = false;
	source:Array<any>;
 ...
 destinationDiffer:any;
  ...
  constructor(public fb: FormBuilder,private differs:IterableDiffers) { 
    this.available = new BasicList(this.AVAILABLE_LIST_NAME);
		this.confirmed = new BasicList(this.CONFIRMED_LIST_NAME);
  }
  ...
  //Data build,movement on action,select all , slecet none
	...
	ngOnChanges(changeRecord: {[key:string]:SimpleChange}) {
		...
		if (changeRecord['format']) {
			...
			if (typeof(this.format.direction) === 'undefined') {
				this.format.direction = this.LTR;
			}

			if (typeof(this.format.add) === 'undefined') {
				this.format.add = this.DEFAULT_FORMAT.add;
			}

			if (typeof(this.format.remove) === 'undefined') {
				this.format.remove = this.DEFAULT_FORMAT.remove;
			}

			if (typeof(this.format.all) === 'undefined') {
				this.format.all = this.DEFAULT_FORMAT.all;
			}

			if (typeof(this.format.none) === 'undefined') {
				this.format.none = this.DEFAULT_FORMAT.none;
			}

			if (typeof(this.format.draggable) === 'undefined') {
				this.format.draggable = true;
			}
		}
	...
	direction() {
		return this.format.direction === 'left-to-right';
	}
